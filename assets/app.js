(() => {
  ...
  function renderHeatMap(){
    ...
    if(!window.L)return;setTimeout(()=>{if(zoneMap){zoneMap.remove();zoneMap=null;}zoneMap=L.map('zoneMap',{zoomControl:true,attributionControl:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(zoneMap);if(!zones.length){zoneMap.setView(defaultCenter,10);return;}const bounds=[];zones.forEach(z=>{const intensity=z.count/maxCount,color=heatMode==='pace'?paceColor(z.avgPace):densityColor(intensity),radius=heatMode==='pace'?Math.max(350,Math.min(950,z.count*125)):300+intensity*1150;bounds.push([z.lat,z.lng]);L.circle([z.lat,z.lng],{radius,color,fillColor:color,fillOpacity:heatMode==='pace'?.3:.22+intensity*.28,weight:heatMode==='pace'?3:1}).bindTooltip(`<b>${z.name}</b><br>${z.count} deliveries<br>${z.avgPace.toFixed(1)} stops/hr`).addTo(zoneMap);if(heatMode==='pace'){const icon=L.divIcon({className:'',html:`<span class="zone-bubble" style="width:30px;height:30px;background:${color}">${z.name.slice(-1)}</span>`,iconSize:[30,30]});L.marker([z.lat,z.lng],{icon}).addTo(zoneMap);}});if(bounds.length>1)zoneMap.fitBounds(bounds,{padding:[30,30]});else zoneMap.setView(bounds[0],14);},60);
  }
