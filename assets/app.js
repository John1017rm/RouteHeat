(() => {
  ...
  let route = null, watchId = null, map = null, userMarker = null, routeLayer = null, timerId = null, historyMap = null, historyReplayLayer = null, zoneMap = null, reportMap = null, undoTimer = null, replayTimer = null;
  ...
  let heatRange = 'all', heatMode = 'density';
  let detailRoute = null, replayEvents = [], replayIndex = 0, reportText = '';
  ...
  function renderHeatMap(){
    const cutoff=heatRange==='all'?0:Date.now()-Number(heatRange)*86400000,filtered=routes().filter(r=>r.startedAt>=cutoff),zones=buildZones(filtered),mapped=zones.reduce((n,z)=>n+z.count,0),maxCount=Math.max(1,...zones.map(z=>z.count)),top=zones[0];
    $('#heatSummary').innerHTML=`<div class="summary-tile"><span>MAPPED STOPS</span><strong>${mapped}</strong></div><div class="summary-tile"><span>ACTIVE AREAS</span><strong>${zones.length}</strong></div><div class="summary-tile"><span>TOP AREA</span><strong>${top?top.name:'—'}</strong></div>`;
    const legend=$('#heatLegend');legend.classList.toggle('pace',heatMode==='pace');legend.firstElementChild.textContent=heatMode==='pace'?'Slower pace':'Fewer deliveries';legend.lastElementChild.textContent=heatMode==='pace'?'Faster pace':'Most deliveries';
    const list=$('#zoneList');list.innerHTML=zones.length?zones.slice(0,8).map(z=>{const color=heatMode==='pace'?paceColor(z.avgPace):densityColor(z.count/maxCount),score=heatMode==='pace'?`${z.avgPace.toFixed(1)}/hr`:`${z.count} stops`,sub=heatMode==='pace'?`${z.count} deliveries recorded`:`${Math.round(z.count/mapped*100)}% of mapped deliveries`;return `<div class="zone-row"><span class="zone-letter" style="background:${color}">${z.name.slice(-1)}</span><div><b>${z.name}</b><small>${sub}</small></div><div class="zone-score"><strong>${score}</strong><span>${heatMode==='pace'?paceClass(z.avgPace):'density'}</span></div></div>`}).join(''):'<div class="empty-state"><p>No GPS-recorded stops in this time range yet.</p></div>';
    if(!window.L)return;setTimeout(()=>{
      if(zoneMap){zoneMap.remove();zoneMap=null;}zoneMap=L.map('zoneMap',{zoomControl:true,attributionControl:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(zoneMap);
      const points=filtered.flatMap(r=>r.stops).filter(s=>s.lat!=null),bounds=points.map(s=>[s.lat,s.lng]);if(!points.length){zoneMap.setView(defaultCenter,10);return;}
      if(heatMode==='density'&&typeof L.heatLayer==='function'){L.heatLayer(points.map(s=>[s.lat,s.lng,1]),{radius:30,blur:27,maxZoom:17,minOpacity:.28,gradient:{0.18:'#2b7fff',0.42:'#24d69a',0.68:'#ffe45c',1:'#ff574d'}}).addTo(zoneMap);}else{zones.forEach(z=>{const color=paceColor(z.avgPace);L.circle([z.lat,z.lng],{radius:Math.max(350,Math.min(950,z.count*125)),color,fillColor:color,fillOpacity:.3,weight:3}).bindTooltip(`<b>${z.name}</b><br>${z.count} deliveries<br>${z.avgPace.toFixed(1)} stops/hr`).addTo(zoneMap);const icon=L.divIcon({className:'',html:`<span class="zone-bubble" style="width:30px;height:30px;background:${color}">${z.name.slice(-1)}</span>`,iconSize:[30,30]});L.marker([z.lat,z.lng],{icon}).addTo(zoneMap);});}
      if(bounds.length>1)zoneMap.fitBounds(bounds,{padding:[30,30]});else zoneMap.setView(bounds[0],15);
    },60);
  }
  function routeEvents(saved){return [...saved.stops.filter(s=>s.lat!=null).map(s=>({...s,type:'stop'})),...(saved.totes||[]).filter(t=>t.lat!=null).map(t=>({...t,type:'tote'}))].sort((a,b)=>a.timestamp-b.timestamp);}
  function drawSavedEvent(saved,event,layer){if(event.type==='stop'){const p=pace(event.segmentMs),prev=saved.stops.filter(s=>s.lat!=null&&s.number<event.number).at(-1);if(prev)L.polyline([[prev.lat,prev.lng],[event.lat,event.lng]],{color:paceColor(p),weight:6,opacity:.92}).addTo(layer);const icon=L.divIcon({className:'',html:`<span class="numbered-marker" style="background:${paceColor(p)}">${event.number}</span>`,iconSize:[28,28]});L.marker([event.lat,event.lng],{icon}).bindTooltip(`Stop ${event.number} · ${p.toFixed(1)}/hr`).addTo(layer);}else{const icon=L.divIcon({className:'',html:`<div class="tote-marker"><span>T${event.number}</span></div>`,iconSize:[30,30]});L.marker([event.lat,event.lng],{icon,zIndexOffset:500}).bindTooltip(`New Tote ${event.number} · after stop ${event.afterStop}`).addTo(layer);}}
  function fitSavedRoute(saved,targetMap){const points=routeEvents(saved).map(e=>[e.lat,e.lng]);if(points.length>1)targetMap.fitBounds(points,{padding:[35,35]});else if(points.length===1)targetMap.setView(points[0],16);else targetMap.setView(defaultCenter,11);}
  function toggleReplay(){if(!detailRoute||!replayEvents.length){toast('No GPS events to replay');return;}if(!historyReplayLayer){toast('Map is still loading');return;}const btn=$('#detailReplayBtn');if(replayTimer){clearInterval(replayTimer);replayTimer=null;btn.textContent='▶ Resume';$('#replayStatus').textContent='Replay paused';return;}if(replayIndex>=replayEvents.length){replayIndex=0;historyReplayLayer.clearLayers();$('#replayProgress').style.width='0%';}if(replayIndex===0)historyReplayLayer.clearLayers();btn.textContent='Ⅱ Pause';const delay=Math.max(130,Math.min(650,Math.round(9000/replayEvents.length)));replayTimer=setInterval(()=>{const event=replayEvents[replayIndex];drawSavedEvent(detailRoute,event,historyReplayLayer);historyMap.panTo([event.lat,event.lng],{animate:true,duration:.35});replayIndex++;const percent=Math.round(replayIndex/replayEvents.length*100);$('#replayProgress').style.width=`${percent}%`;$('#replayStatus').textContent=event.type==='stop'?`Stop ${event.number} of ${detailRoute.stops.length}`:`New Tote ${event.number}`;if(replayIndex>=replayEvents.length){clearInterval(replayTimer);replayTimer=null;btn.textContent='↻ Replay again';$('#replayStatus').textContent='Route replay complete';}},delay);}
  function showRouteDetail(id){
    ...
    detailRoute=saved;replayEvents=routeEvents(saved);replayIndex=0;clearInterval(replayTimer);replayTimer=null;$('#detailReplayBtn').textContent='▶ Replay';$('#replayProgress').style.width='0%';$('#replayStatus').textContent='Full route';
