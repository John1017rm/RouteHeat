(() => {
  ...
  let route = null, watchId = null, map = null, userMarker = null, routeLayer = null, timerId = null, historyMap = null;
  ...
  function showRouteDetail(id){
    const saved=routes().find(r=>r.id===id); if(!saved)return; const ms=(saved.endedAt||Date.now())-saved.startedAt, overall=saved.stops.length?pace(ms/saved.stops.length):0;
    ...
    $('#detailStops').innerHTML=saved.stops.length?`<div class="stop-list">${saved.stops.slice().reverse().map(s=>{const p=pace(s.segmentMs);return `<div class="stop-row"><span class="stop-number">${s.number}</span><div class="stop-info"><b>Stop ${s.number}</b><span>${timeLabel(s.timestamp)} · ${formatDuration(s.segmentMs).slice(3)} segment</span></div><span class="pace-pill ${paceClass(p)}">${p.toFixed(1)}/hr</span></div>`}).join('')}</div>`:'<div class="empty-state"><p>No stops were logged on this route.</p></div>';
