(() => {
  ...
  let autoEnabled = localStorage.getItem('routeheat.autoDetect') === 'true';
  ...
  function startRoute() {
    ...
    const now=Date.now(); route={id:`rh-${now}`,startedAt:now,endedAt:null,stops:[],lastPosition:null,paceGoal,plannedStops,pauses:[],pausedAt:null}; $('#goalModal').classList.remove('open'); resetDetector();
    $('#routeToggle').classList.add('running'); $('#routeToggle').innerHTML='<span class="play">■</span><span>Finish</span>';
    $('#pauseBtn').hidden=false; $('#routeTitle').textContent='Route in progress'; $('#stopBtn').disabled=false; $('#stopButtonHint').textContent='Stop 1 • tap once when parked';
  ...
  function exportCsv(items){
    const rows=[['route_id','route_date','route_started','route_ended','stop_number','stop_source','stop_time','segment_seconds','segment_stops_per_hour','latitude','longitude','gps_accuracy_m']];
    items.forEach(r=>r.stops.forEach(s=>rows.push([r.id,new Date(r.startedAt).toLocaleDateString(),new Date(r.startedAt).toISOString(),new Date(r.endedAt).toISOString(),s.number,s.source||'manual',new Date(s.timestamp).toISOString(),Math.round(s.segmentMs/1000),pace(s.segmentMs).toFixed(2),s.lat??'',s.lng??'',s.accuracy?Math.round(s.accuracy):''])));
  ...
  $('#routeToggle').addEventListener('click',()=>route?askFinish():openGoalSetup()); $('#pauseBtn').addEventListener('click',togglePause); $('#stopBtn').addEventListener('click',()=>logStop('manual')); $('#locateBtn').addEventListener('click',()=>getPosition(true));
